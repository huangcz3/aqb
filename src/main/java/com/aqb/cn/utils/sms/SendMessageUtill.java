...
public class SendMessageUtill {
    ...
    public static HashMap<String,String> getIdentifying_code(String phone) {
        ...
        String content = new String("您的验证码是：" + mobile_code  + "【弹幕猎人】");
        //设置发送内容的编码方式
        try {
            String send_content=URLEncoder.encode(content.replaceAll("<br/>", " "), "UTF-8");//发送内容
            NameValuePair[] data = {//提交短信
                    new NameValuePair("userid",""),
                    new NameValuePair("account", "jksc889"), //账号
                    new NameValuePair("password", "q789789"),  //密码
        ...
        try {
            ...
            String status = root.elementText("returnstatus");//状态
            ...
            m.put("remainpoint",remainpoint);
            m.put("mobile_code",String.valueOf(mobile_code));
...
//            }
    ...
    public static void main(String[] args) {
        ...
        String xml=null;
        SendMessageUtill sms = new SendMessageUtill();

        //查询调用方法
        xml=sms.SelSum("", "wa21", "a123123").toString();
        //输出xml形式的字符串
        ...
        //调用XML字符串解析通用方法
        xmlentity=sms.readStringXmlCommen(xmlentity, xml);
        ...
        //发送调用
        xml=sms.SendMessage("", "wa21", "a123123", "18328005498", "测试剩余与使用1", "").toString();
...
//		System.out.println("错误代码："+xmlentity.getError());
    ...
    {
        try {
            url=new URL("http://sh2.cshxsp.com/sms.aspx?action=overage&userid="+userid+"&account="+account+"&password="+password+"");
    ...
    public static StringBuffer SendMessage(String userid,String account,String password,String mobile,String content,String sendTime)
    {
        ...
        try {
            ...
            url=new URL("http://sh2.cshxsp.com/sms.aspx?action=send&userid="+userid+"&account="+account+"&password="+password+"&mobile="+mobile+"&content="+send_content+"&sendTime="+sendTime+"");
            con = (HttpURLConnection)url.openConnection();
    ...
    {
        try {
            url=new URL("https://dx.ipyy.net/sms.aspx?action=query&userid="+userid+"&account="+account+"&password="+password+"");
            con = (HttpURLConnection)url.openConnection();
